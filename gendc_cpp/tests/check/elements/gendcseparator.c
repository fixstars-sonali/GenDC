...
create_pipeline (GstPadMode mode, gchar * path)
{
  ...
  gendcseparator = gst_element_factory_make ("gendcseparator", "gendcseparator");
  fail_if (gendcseparator == NULL);
  fakesink = gst_element_factory_make ("fakesink", "fakesink");
  fail_if (fakesink == NULL);

  gst_bin_add_many (GST_BIN (pipeline), src, gendcseparator, fakesink, q, NULL);

  g_object_set (src, "location", path, NULL);
  // g_object_set (src, "location", SIMPLE_GENDC_PATH, NULL);

  if (mode == GST_PAD_MODE_PUSH)
    fail_unless (gst_element_link_many (src, q, gendcseparator, fakesink, NULL));
  else
    fail_unless (gst_element_link_many (src, gendcseparator, fakesink, NULL));
...
do_test_empty_file (gboolean can_activate_pull)
{
  ...
  fail_if (src == NULL);
  gendcseparator = gst_element_factory_make ("gendcseparator", NULL);
  fail_if (gendcseparator == NULL);
  fakesink = gst_element_factory_make ("fakesink", NULL);
  fail_if (fakesink == NULL);

  gst_bin_add_many (GST_BIN (pipeline), src, gendcseparator, fakesink, NULL);
...
static Suite *
gendcseparator_suite (void)
{
  Suite *s = suite_create ("gendcseparator");
  TCase *tc_chain = tcase_create ("gendcseparator");
